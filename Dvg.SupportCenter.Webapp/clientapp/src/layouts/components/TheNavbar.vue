  ...
  export default {
    ...
    computed: {
      ...
      user_displayName() {
        ...
        let name = this.$store.state.auth.currentUser.name;
        return name;
      },
      activeUserImg() {
        // return JSON.parse(localStorage.getItem('userInfo')).photoURL || this.$store.state.AppActiveUser.img;
        var avatarUrl = this.$store.state.auth.currentUser.avatarUrl;
        if (!avatarUrl)
          return "/images/support-avatar-default.png";
        return avatarUrl;
      }
    },
    methods: {
