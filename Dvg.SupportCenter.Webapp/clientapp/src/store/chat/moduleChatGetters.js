...
export default {
    ...
    chats: (state, getters) => {
        const chatArray = state.contacts.filter((contact) => {
    ...
    activeContact: (state) => {
        let activeContact = state.contactManager.contacts.find(contact => contact.username == state.contactManager.activeContactUsername);
        return activeContact;
    },
    activeConversation: (state) => {
        let activeContact = state.contacts.find(contact => contact.active);
        let conversation = state.conversations[activeContact.id];
